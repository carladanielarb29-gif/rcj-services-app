...
export async function POST(req: Request) {
  ...
  const { fullName, email, phone, company, taxId, message } = validation.data;
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      <p>
        El usuario <strong>${escapeHtml(fullName)}</strong> ha presentado una Queja a través de la plataforma de RCJ Services.
      </p>
    ...
    if (error) {
      console.error("Resend error sending complaint email:", error);
      return Response.json({ error: "No se pudo enviar el correo." }, { status: 502 });
    }

    return Response.json(data);
  } catch (err) {
    console.error("Unexpected error sending complaint email:", err);
